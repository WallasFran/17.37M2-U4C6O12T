...
void ssd1306_draw_char(ssd1306_t *ssd, char c, uint8_t x, uint8_t y)
{
  ...
  if (c >= 'A' && c <= 'Z')
  {
    index = (c - 'A' + 11) * 8;  // Para letras maiúsculas (A começa no índice 11)
  }
  // Se for uma letra minúscula
  else if (c >= 'a' && c <= 'z')
  {
    index = (c - 'a' + 37) * 8;  // Para letras minúsculas (a começa no índice 37)
  }
  // Se for um número
  else if (c >= '0' && c <= '9')
  {
  ...
  {
    uint8_t line = font[index + i];  // Pega a linha do caractere da tabela de fontes
    for (uint8_t j = 0; j < 8; ++j)
    {
      ssd1306_pixel(ssd, x + i, y + j, line & (1 << j));  // Desenha o pixel no display
    }
  }
}
